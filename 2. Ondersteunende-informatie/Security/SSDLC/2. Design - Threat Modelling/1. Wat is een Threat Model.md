---
title: 1. Wat is een Threat Model
---
# Threat modeling
...
**!!De onderstaande foto is geen goed voorbeeld van een threat model, deze + de tekst moet nog worden aangepast!!**
...
Een goed voorbeeld van waar trust boundaries zich kunnen bevinden is tussen een browser en een applicatie. In het onderstaande voorbeeld ligt de trust boundary tussen de `browser` en `Project Vanilla Javascript` en `Project .NET`. Dit betekent dat er een potentieel beveiligingsrisico bestaat tussen deze onderdelen.

Opvallend is dat er geen trust boundary aanwezig is tussen `Project .NET` en de `Database`. Dit gaat er dus vanuit dat de API-endpoints de enige toegang tot de database zijn. Echter, in de praktijk kan iemand via de computer waarop `Project .NET` draait, toegang krijgen tot de database. Hoewel het onderstaande schema dit niet aangeeft, is dit wel een reëel scenario. Daarom zou je ervoor kunnen kiezen om een trust boundary te plaatsen tussen `Project .NET` en de `Database`. Dit zorgt ervoor dat ook deze risico's worden meegenomen in het beveiligingsrapport en de uiteindelijke maatregelen.
...
1. **Risicoanalyse**: Prioriteren van de geïdentificeerde bedreigingen op basis van hun [waarschijnlijkheid](2.%20Ondersteunende-informatie/Security/SSDLC/1.%20Requirements%20-%20Risk%20Assessment/2.%20Test#waarschijnlijkheid) en [impact](2.%20Ondersteunende-informatie/Security/SSDLC/1.%20Requirements%20-%20Risk%20Assessment/2.%20Test#impact).
...
Meer over de risico analyse is te vinden in [1. Wat is een Risk Assessment](2.%20Ondersteunende-informatie/Security/SSDLC/1.%20Requirements%20-%20Risk%20Assessment/1.%20Wat%20is%20een%20Risk%20Assessment).
### Mitigatieplanning
...
Er zijn veel tools voor het maken van een threat model beschikbaar. Binnen HBO-ICT wordt voornamelijk gebruikt gemaakt van [Microsoft Threat Modeling Tool](https://learn.microsoft.com/en-us/azure/security/develop/threat-modeling-tool).  Een uitleg van deze tool is te vinden op deze pagina: [Handleiding Microsoft Threat Modeling Tool](2.%20Ondersteunende-informatie/Security/Handleiding%20Microsoft%20Threat%20Modeling%20Tool). Een alternatief is [OWASP Threat Dragon](https://owasp.org/www-project-threat-dragon/). 
## Begrippenlijst
